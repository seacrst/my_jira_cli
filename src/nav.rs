    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            Action::NavigateToEpicDetail { epic_id } => {
                let epic_det = EpicDetail {epic_id, db: Rc::clone(&self.db) };
                self.pages.push(Box::new(epic_det));
            }
            
            Action::NavigateToStoryDetail { epic_id, story_id } => {
                let story_det = StoryDetail {db: Rc::clone(&self.db), story_id, epic_id};
                self.pages.push(Box::new(story_det));
            }
            ...
            Action::CreateEpic => {
                let prompt = &self.prompts.create_epic;
                self.db.create_epic(prompt()).with_context(|| anyhow!("No epic has been created"))?;
            }
            ...
            Action::UpdateEpicStatus { epic_id } => {
                ...
                if let Some(s) = status {
                    self.db.update_epic_status(epic_id, s).with_context(|| anyhow!("Cannot update epic's status"))?;
                }
            }
            
            Action::DeleteEpic { epic_id } => {
                let del = (self.prompts.delete_epic)();
                if del {
                    self.db.delete_epic(epic_id).with_context(|| anyhow!("Could not delete epic"))?;
            ...
            Action::CreateStory { epic_id } => {
                ...
                self.db.create_story(story, epic_id).with_context(|| anyhow!("Could not create story"))?;
            }
            ...
            Action::UpdateStoryStatus { story_id } => {
                ...
                if let Some(s) = status {
                    self.db.update_story_status(story_id, s).with_context(|| anyhow!("Could not update status"))?;
                }
            }

            Action::DeleteStory { epic_id, story_id } => {
                let del = (self.prompts.delete_story)();
                if del {
                    self.db.delete_story(epic_id, story_id).with_context(|| anyhow!("Could not delete story"))?;
